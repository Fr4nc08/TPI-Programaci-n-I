...
def jugar():
   ...
   with open("usuarios.txt", "a") as archivo_usuarios:
    ...
    # Pedimos que ingresen su nombre y lo guardamos en el archivo de usuario
   puntaje = 0
   suma = 0
   # Generamos la variable de la suma que luego se realizará y una para el puntaje.<
   for i in range(5):
      ...
      try:
        ...
        continue
      suma = num1 + num2
      # Hacemos la suma de los números creados
   ...
   with open("puntajes.txt", "a") as archivo_puntajes:
    archivo_puntajes.write(f"{usuario}: {puntaje} puntos\n")
    # Se muestra en pantalla el puntaje obtenido y se guarda el mismo en el archivo junto al nombre
adivina_la_suma()
while True:
